...
TMP_FOLDER=$(mktemp -d)
CONFIG_FILE='sagiso.conf'
CONFIGFOLDER='/root/.sagiso'
COIN_DAEMON='sagisod'
COIN_CLI='sagiso-cli'
COIN_TX='sagiso-tx'
COIN_PATH='/usr/local/bin/'
OS_VERSION=$(lsb_release -d)
COIN_TGZP='https://github.com/Sagiso/sagiso/releases/download/1000/SAGISO-1.0.0.-Linux.zip'
COIN_BOOTSTRAP='https://github.com/Sagiso/sagiso/releases/download/1000/bootstrap.zip'
...
function create_key() {
  echo -e "${YELLOW}Enter your ${RED}$PROJECT_NAME ${GREEN}Masternode Private Key${YELLOW} produced on your local wallet by ${RED}createmasternodekey${YELLOW} command or press enter for a ${GREEN}Masternode Private Key${YELLOW} is generated automatically${NC}"
  read -e COINKEY
  if [[ -z "$COINKEY" ]]; then
...
cat << EOF > /root/sagiso-general-info
echo -e "\n\n${BLUE}=======================================================================================================${NC}\n"
echo -e "${GREEN}$PROJECT_NAME General Info: ${NC}"
$COIN_CLI getinfo
echo -e "\n${BLUE}=======================================================================================================${NC}\n"
EOF
chmod +x /root/sagiso-general-info

cat << EOF > /root/sagiso-fee-info
echo -e "\n\n${BLUE}=======================================================================================================${NC}\n"
echo -e "${GREEN}$PROJECT_NAME Fee Info: ${NC}"
$COIN_CLI getfeeinfo 100
echo -e "\n${BLUE}=======================================================================================================${NC}\n"
EOF
chmod +x /root/sagiso-fee-info

cat << EOF > /root/sagiso-networkinfo
echo -e "\n\n${BLUE}=======================================================================================================${NC}\n"
echo -e "${GREEN}$PROJECT_NAME Network Info: ${NC}"
$COIN_CLI getnetworkinfo
echo -e "\n${BLUE}=======================================================================================================${NC}\n"
EOF
chmod +x /root/sagiso-networkinfo

cat << EOF > /root/sagiso-masternode-stats
echo -e "\n\n${BLUE}=======================================================================================================${NC}\n"
...
echo -e "\n${BLUE}=======================================================================================================${NC}\n"
EOF
chmod +x /root/sagiso-masternode-stats

cp /root/.bashrc /root/.bashrc.backup
sed '/feestats/d' /root/.bashrc | sed '/networkstats/d' | sed '/mnstats/d' | sed '/sagisoinfo/d' > /root/tmp
mv /root/tmp /root/.bashrc
echo -e "alias sagisoinfo='/root/sagiso-general-info'" >> /root/.bashrc
echo -e "alias feestats='/root/sagiso-fee-info'" >> /root/.bashrc
echo -e "alias networkstats='/root/sagiso-networkinfo'" >> /root/.bashrc
echo -e "alias mnstats='/root/sagiso-masternode-stats'" >> /root/.bashrc
exec bash
}
...
function important_information() {
 ...
 echo -e "You can also use ${RED}sagiso-cli getinfo${NC}, ${RED}sagiso-cli getnetworkinfo${NC}, ${RED}sagiso-cli getconnectioncount${NC}commands for a nice looking infos.${NC}"
 echo -e "${BLUE}================================================================================================================================${NC}"
